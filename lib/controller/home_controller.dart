...
class HomeController extends GetxController {
  ...
  FirebaseFirestore.instance
    ...
    tweets.value = snapshot.docs.map((doc) {
  ...
  print("Posting tweet with image: ${base64Image.value.isNotEmpty ? 'Yes (${base64Image.value.length} chars)' : 'No'}");

  CollectionReference tweetsRef = FirebaseFirestore.instance.collection('tweets');
 ...
 Future<void> selectImage(XFile image) async {
  ...
  void updateTweetState() {
     isTweetEnabled.value = tweetController.text.isNotEmpty || base64Image.value.isNotEmpty;
  }
...
Future<void> deleteTweet(String tweetId) async {
  try {
    ...
    print("Tweet $tweetId deleted successfully");
    Get.snackbar(
      'Success', 
      'Tweet deleted',
