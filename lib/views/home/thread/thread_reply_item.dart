...
class ThreadReplyItem extends StatelessWidget {
  ...
  ThreadReplyItem({Key? key, required this.reply, this.onTap, required this.threadRootId})
    : super(key: key);
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: () {
        // Menggunakan onTap callback jika disediakan
        if (onTap != null) {
          onTap!();
        } else {
          // Jika tidak, gunakan fungsi default untuk mengganti headline
          tweetController.changeThreadHeadline(
            reply.id, 
            threadRootId
          );
        }
        // Force rebuild tampilan
        tweetController.forceRefresh();
        print('Navigating to thread with ID: ${reply.id}');
      },
      child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        ...
                        GestureDetector(
                          ...
                          child: _buildAction('assets/images/tweet_retweet.png', ''),
                        ),
                        SizedBox(width: 50),
                        _buildAction(
  ...
  void showRetweetBottomSheet() {
    Get.bottomSheet(
      BottomSheetRetweet(tweet: reply),
